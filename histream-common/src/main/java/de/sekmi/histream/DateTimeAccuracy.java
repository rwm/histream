...
//	 */
	...
	private static int appendPartialDate(StringBuilder b, TemporalAccessor dt, ChronoUnit accuracy) {
		char[] prefixes = {0,'-','-','T',':',':','.'};
	...
	public String toPartialLocalIso(ZoneId tz) {
		Objects.requireNonNull(tz);
		StringBuilder b = new StringBuilder(20);
		if( instant == null )return "null";
		TemporalAccessor dt;
		dt = instant.atOffset(ZoneOffset.UTC).atZoneSameInstant(tz);
		appendPartialDate(b, dt, accuracy);
		return b.toString();
	}

	/**
	 * Returns a local date+time object. The returned time is always full as
	 * it does not respect the accuracy.
	 * @param tz timezone where the local time should be calculated
	 * @return local time
	 */
	public LocalDateTime toLocal(ZoneId tz) {
//		return instant.atOffset(ZoneOffset.UTC).atZoneSameInstant(tz).toLocalDateTime();
		return LocalDateTime.ofInstant(instant, tz);
	}

	private void appendZoneOffset(StringBuilder b, int os, boolean colon) {
		// os - offset seconds
		if( os == 0 ){
			// output Z
			b.append('Z');
	...
	public String toPartialIso8601(ZoneId tz){
		...
		int i = appendPartialDate(b, dt, accuracy);
	 ...
	 * @return String formatted partial date time
	 */
	public String toFhirDateTime(ZoneId tz) {
		StringBuilder b = new StringBuilder(20);
		if( instant == null )return "null";
		...
		// for FHIR dateTime, seconds must be always provided due to schema constraints
		ChronoUnit outputAccuracy = accuracy;
		if( outputAccuracy == ChronoUnit.HOURS
				|| outputAccuracy == ChronoUnit.MINUTES )
		{
			// increase output accuracy to seconds
			outputAccuracy = ChronoUnit.SECONDS;
		}
		int i = appendPartialDate(b, dt, outputAccuracy);
